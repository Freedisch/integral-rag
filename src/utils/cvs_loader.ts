...
export class CsvLoader {
  ...
  async loadNetworks(): Promise<Network[]> {
    return this.loadCsv<Network>(
      path.join(this.pathDir, 'Networks.csv'),
      row => ({
        id: parseInt(row.id),
        name: row.name,
      }),
  ...
  async loadProfiles(): Promise<Profile[]> {
    return this.loadCsv<Profile>(
      path.join(this.pathDir, 'Profiles.csv'),
      row => ({
        id: parseInt(row.id),
        name: row.name,
        bio: row.bio,
  ...
  async loadMembers(): Promise<Members[]> {
    return this.loadCsv<Members>(
      path.join(this.pathDir, 'Members.csv'),
      row => ({
        id: parseInt(row.id),
